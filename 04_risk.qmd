...
# Check temperature units and convert from Kelvin if needed
if (max(era5_delu$temperature_2m, na.rm = TRUE) > 100) {
  era5_delu <- era5_delu |> mutate(temperature_2m = temperature_2m - 273.15)
}
...
# ── Stage 3 price models ─────────────────────────────────────────────
# Full-sample lasso (primary model for Variant A/B predictions)
price_model      <- readRDS("models/price_lasso_DE-LU.rds")
# Post-crisis lasso (2023-2025 only — for regime comparison)
price_model_post <- readRDS("models/price_lasso_post_DE-LU.rds")
# Quantile XGBoost (for distributional predictions)
price_xgb_q05    <- readRDS("models/price_xgb_q05_DE-LU.rds")
price_xgb_q95    <- readRDS("models/price_xgb_q95_DE-LU.rds")
```
...
We reconstruct the same feature set used in the Stage 3 price model: renewable share, residual load, net export balance, TTF gas price (level and daily change), thermal generation, solar irradiance, HDD/CDD, renewable ramp rate, holidays, hour, day-of-week, and season.
...
scenario_df <- smard |>
  ...
  # Join ERA5 weather (ssrd, HDD, CDD)
  left_join(
    era5_delu |> select(datetime_utc, ssrd_wm2, hdd, cdd),
    by = "datetime_utc"
  ) |>
...
scenario_df <- scenario_df |>
  ...
  mutate(
    ...
    hdd                = na_kalman(hdd, model = "StructTS"),
    cdd                = na_kalman(cdd, model = "StructTS"),
...
n_remaining_na <- scenario_df |>
  select(renewable_share, residual_load, net_export_mw, ttf_eur_mwh,
         ttf_change, thermal_gen, ssrd_wm2, hdd, cdd) |>
...
#| fig-cap: "Monthly revenue distributions by season and simulation variant. Realized uses actual historical prices; the model-based variants show how much risk the weather-only and full models capture."
#| fig-height: 8
...
## Post-Crisis Comparison (2023-2025) {#sec-comp-post-crisis}
...
```{r}
#| label: fig-partial-r2-post-comparison
#| fig-cap: "Post-crisis (2023-2025): variance attribution shifts as gas prices stabilise. Renewable share and residual load gain importance relative to TTF."
#| fig-height: 7
#| fig-width: 10
...
```{r}
#| label: tbl-partial-r2-post-summary
...
Are `r comma(params$n_sim)` simulations enough for stable risk estimates? The plot below shows how CVaR 95% on realized revenue stabilises as the number of draws increases.

```{r}
#| label: fig-convergence-check
#| fig-cap: "CVaR 95% on realized revenue converges well before the full simulation count"
#| fig-height: 5
